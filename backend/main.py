...
app = FastAPI(title="Anna MVP API", version="0.3.6")
...
class Intake(BaseModel):
    ...
    # Let op: gender is *optioneel*. We accepteren ook 'geslacht' uit de frontend en mappen dit.
    gender: Optional[str] = Field(None, description="man of vrouw (optioneel)")
    ...
    if HAS_PYDANTIC_V2:
        ...
        def map_geslacht_to_gender(cls, values):
            """Zet 'geslacht' (man/vrouw) om naar 'gender' vóór validatie (v2)."""
            if isinstance(values, dict):
                ...
                if g in {"male", "m"}:   g = "man"
                if g in {"female", "f"}: g = "vrouw"
                if g: data["gender"] = g
                return data
            return values
    else:
        ...
        def map_geslacht_to_gender(cls, values):
            """Zet 'geslacht' (man/vrouw) om naar 'gender' vóór validatie (v1)."""
            if isinstance(values, dict):
...
def _alloc(budget: float) -> Dict[str, float]:
    return {
        ...
        "_total": budget * 1.00,  # indicatief
    }

def _gender_token(g: Optional[str]) -> str:
    """Converteer man/vrouw (en tolerante varianten) naar 'men' of 'women'."""
    if not g:
        return ""
    g = str(g).strip().lower()
    if g in {"man", "male", "m"}:     return "men"
    if g in {"vrouw", "female", "f"}: return "women"
    return ""
...
def _build_query(category: str, intake: Dict[str, Any]) -> str:
    ...
    gender = _gender_token(intake.get("gender"))  # ← optioneel
...
def _map_item(category: str, r: dict, link: str, price: float) -> dict:
    ...
    return {
        "category": category, "title": title, "price": round(price, 2),
        "currency": cur, "link": link, "image": img,
        "merchant": merchant, "cheaper_alternative": None,
    }
    ...
    for i in range(max(1, min(outfits_count, 6))):
        ...
        for cat in cats:
            ...
            if r:
                ...
                item = {
                    "category": cat, "title": "(geen directe shoplink gevonden — alternatief)",
                    "price": round(alloc[cat], 2), "currency": "EUR",
                    "link": "#", "image": None, "merchant": "—", "cheaper_alternative": None,
                }
            items.append(item); total += float(item.get("price") or 0)
        outfits.append({"name": f"Outfit {i+1}", "items": items, "total": round(total, 2), "currency": items[0].get("currency","EUR")})

    return {
        "palette": palette, "allocation": alloc, "outfits": outfits,
